...
function readPricingConfig(
  ...
  const imageOutputRatio = lookupModelRatio(settings.ImageOutputRatio, modelName)
      ...
      try {
        ...
        if (response.success) {
          ...
          if (modelSettings) {
            ...
            const newImageOutputRatio = normalizeJsonString(JSON.stringify(imageOutputMap))
            if (
              newImageOutputRatio !== normalizeJsonString(modelSettings.ImageOutputRatio)
            ) {
              updates.push({ key: 'ImageOutputRatio', value: newImageOutputRatio })
            }
                  ...
                  >
                    ...
                    <CollapsibleContent className='flex flex-col gap-4 pt-4'>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>{t('Image output ratio')}</FormLabel>
                            <FormControl>
                              <Input
                                type='text'
                                placeholder='1.0'
                            ...
                            <FormDescription>
                              {t('Multiplier for image output processing.')}
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
